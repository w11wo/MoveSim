...
class Discriminator(nn.Module):
    ...
    def __init__(self, total_locations=8606, embedding_net=None, embedding_dim=64, dropout=0.6):
        ...
        if embedding_net:
            ...
            self.embedding = nn.Embedding(
                num_embeddings=total_locations + 1, embedding_dim=embedding_dim, padding_idx=total_locations
            )
...
class TCDiscriminator(nn.Module):
    ...
    def __init__(self, total_locations=8606, embedding_net=None, sembedding_dim=64, tembedding_dim=16, dropout=0.6):
        ...
        if embedding_net:
            ...
            self.tembedding = nn.Embedding(num_embeddings=total_locations, embedding_dim=tembedding_dim)
            self.sembedding = nn.Embedding(num_embeddings=total_locations, embedding_dim=sembedding_dim)
        self.convs = nn.ModuleList(
            [nn.Conv2d(1, n, (f, tembedding_dim + sembedding_dim)) for (n, f) in zip(num_filters, filter_sizes)]
        )
        self.highway = nn.Linear(sum(num_filters), sum(num_filters))
        self.dropout = nn.Dropout(p=dropout)
        self.linear = nn.Linear(sum(num_filters), 2)
